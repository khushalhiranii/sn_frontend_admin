...
function Kycprocess() {
  ...
  const getMergedData = (status) => {
    ...
    const filteredUsers = usersArray.filter((user) => {
      ...
      const account = accountArray.find((a) => a.Identifier === user.Identifier);
      ...
      return (
        ...
        account?.Status === false // Ensure account status is false
      );
    });
  ...
  const mergedAccounts = selectedType === 'Approval Request' ? getMergedData(true) : getMergedData(false);
        ...
        <div className="self-stretch rounded-xl bg-white box-border flex flex-col items-start justify-start pt-[0rem] pb-[1rem] gap-[1rem] max-w-full shrink-0 z-[1] text-[1rem] text-foundation-blue-normal font-roboto border-[1px] border-solid border-foundation-white-normal-hover">
          ...
          <div className="w-full flex flex-row items-center justify-between px-[16px] box-border">
            {mergedAccounts.map((account, index) => (
              <SavingCard key={index} phoneno={account.Number} fullname={account.Name} address={account.Address} profilePicture={account.Photo} key1={account.Identifier} account={account.Account} status={account.Status} />
            ))}
          </div>
        </div>
      </div>
